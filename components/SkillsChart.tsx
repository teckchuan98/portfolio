      ...
      <div className="container mx-auto px-6">
        <ScrollReveal width="100%">
          <div className="text-center mb-16">
            ...
            <p className="mt-4 text-slate-400">A visual overview of my technical arsenal.</p>
          </div>
        </ScrollReveal>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
             {SKILLS.map((skill, index) => (
               <ScrollReveal key={skill.name} delay={300 + (index * 50)} width="100%">
                 <div className="bg-slate-800/50 p-5 rounded-xl border border-slate-700 hover:border-primary-500/50 transition-all duration-300 group hover:bg-slate-800 hover:transform hover:-translate-y-1 hover:shadow-lg">
                   <div className="flex justify-between items-center mb-3">
                     <h3 className="font-semibold text-white group-hover:text-primary-400 transition-colors">{skill.name}</h3>
                   ...
                   <div className="flex items-center gap-3">
                      <div className="flex-1 bg-slate-700 h-2 rounded-full overflow-hidden">
                        <div 
                            className="bg-gradient-to-r from-primary-600 to-primary-400 h-full rounded-full transition-all duration-1000 ease-out group-hover:animate-pulse" 
                            style={{ width: `${skill.level}%` }}
                        ></div>
                      </div>
                      <span className="text-xs font-mono text-slate-400 w-8 text-right">{skill.level}%</span>
                   </div>
                 </div>
               </ScrollReveal>
             ))}
